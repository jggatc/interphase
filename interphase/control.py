...
class Control(object):
    ...
    **Control Object**
    ...
    def __init__(self,
        ...
        setting = self._set_listing(control_list, icon_list, self.size)
        self.listing = setting[0]
        self.value = setting[1]
        self.control_icon = setting[2]
        self.icon_size = setting[3]
        self.size = setting[4]
        ...
        pos_x = pos_x - (self.size[0]//2)
        pos_y = pos_y - (self.size[1]//2)
        self.position = int(pos_x), int(pos_y)
        self.display = self.set_display_info(
            self.size, self.font_color, self.font_type, self.font_size,
            center=self.center_text)
        self.label = self.set_label_info(
            self.size, self.font_color, self.font_type, self.font_size)
        if label:
        ...
        self.text_image = {}
        self.button, self.rects = self._define_buttons(
            self.control_type, self.size, color, fill)
        ...
        if self.listing:
            if (len(self.listing) > self.hold_response_set//100 or
                (self.listing[0][:-2] == '__numeric' and
                 abs(self.numeric['h']-self.numeric['l']) >
                  self.hold_response_set//100) and
                self.control_response > self.control_response_hold):
        ...
        self.text_margin = {}
        if isinstance(text_margin, int):
            self.text_margin['t'] = text_margin
            self.text_margin['r'] = text_margin
            self.text_margin['b'] = text_margin
            self.text_margin['l'] = text_margin
        else:
            self.text_margin['t'] = text_margin[0]
            self.text_margin['r'] = text_margin[1]
            self.text_margin['b'] = text_margin[2]
            self.text_margin['l'] = text_margin[3]
    ...
    def _define_buttons(self, control_type, size,
                        color, fill, initialize=True):
        ...
        if self.switches:
            ...
            if not self.control_image:
                button[self.id] = lambda: engine.draw.rect(
                    self.panel.image, self.active_color,
                    (self.position,size), fill)
            else:
                background = engine.transform.smoothscale(
                    self.control_image['bg'], size)
                button[self.id+'_bg'] = lambda: self.panel.image.blit(
                    background, self.position)
                button[self.id] = lambda: engine.draw.rect(
                    self.panel.image, self.active_color,
                    (self.position,size), fill)
            if not self.panel._button_image:
                button[self.id+'_top'] = lambda: engine.draw.polygon(
                    self.panel.image, color,
                    ((x1, y1-10),(x1-5, y1),(x1+5, y1)), fill)
            else:
                button[self.id+'_top'] = lambda: self.panel.image.blit(
                    self.panel._button_image['t'],
                    (x1 - self.panel._button_size[0]//2,
                     y1 - self.panel._button_size[1]))
            if not self.panel._button_image:
                button[self.id+'_bottom'] = lambda: engine.draw.polygon(
                    self.panel.image, color,
                    ((x2, y2+10),(x2-5, y2),(x2+5, y2)), fill)
            else:
                button[self.id+'_bottom'] = lambda: self.panel.image.blit(
                    self.panel._button_image['b'],
                    (x2 - self.panel._button_size[0]//2, y2))
        else:
            if not self.control_image:
                button[self.id] = lambda: engine.draw.rect(
                    self.panel.image, self.active_color,
    ...
    def _define_button_initialize(self, button):
        ...
        for btn in button:
            if btn not in self.panel._controls_disabled:
                offset_x = self.panel._panel_rect[0]
                offset_y = self.panel._panel_rect[1]
                rect = button[btn]()
                rect[0] = rect[0] + offset_x
                rect[1] = rect[1] + offset_y
                rects[btn] = rect
        return button, rects
    ...
    def _display(self, image):
        for btn in self.button_list:
            self.button[btn]()
        if (self.list_type.startswith('__') or
            not self.value.startswith('__')):
    ...
    def _set_listing(self, control_list=None, icon_list=None,
                     size='auto', case=False,
                     data_folder=None, data_zip=None, file_obj=None,
                     color_key=None, surface=None):
        ...
        if control_list:
            if control_list[0] == '__numeric':
                ...
                if icon_list or surface:      #icon in listing
                    ...
                    if listing_icon:
                        control_icon = self._set_listing_icon(
                            listing_icon, icon_list, data_folder,
                            data_zip, file_obj, color_key, surface)
        self.panel._control_values[self.id] = self.value
        self.size, self.icon_size = self._set_control_size(
            self.listing, control_icon, size, self.font_size)
        if control_icon:
            self.control_icon = self._set_icon_size(
                control_icon, self.icon_size)
        return (self.listing, self.value, self.control_icon,
                self.icon_size, self.size)

    def _set_listing_icon(self, listing_icon, icon_list=None,
                          data_folder=None, data_zip=None, file_obj=None,
                          color_key=None, surface=None):
        ...
        if icon_list:
            data_folder, data_zip, color_key = self.panel._data_source(
                data_folder, data_zip, color_key, file_obj)
            if len(icon_list) == 1 and frames > 1:
                images = self.panel._load_image(
                    icon_list[0], frames, path=data_folder,
                    zipobj=data_zip, fileobj=file_obj, colorkey=color_key)
                for num, item in enumerate(listing_icon):
                    control_icon[item] = images[num]
            else:
                for num, item in enumerate(listing_icon):
                    img = self.panel._load_image(
                        icon_list[num], path=data_folder,
            ...
            if not (width == size[0] and height == size[1]):
                if width != size[0]:
                    ...
                    if w > size[0]:
                        w = size[0]
                        h = int((w/width) * height)
                control_icon[icon] = engine.transform.smoothscale(
                    control_icon[icon], (w,h))
        return control_icon
    ...
    def _set_control_size(self, listing, control_icon, size, font_size):
        ...
        if not isinstance(size, str):
            ...
            if listing:
                text_display = self.panel._text(self.panel.image,
                                                self.font_type)
                ...
                if size in ('auto'):
                    ...
                    if self.panel._control_minsize:
                        ...
                        if width/height > minsize['size'][0]/minsize['size'][1]:
                            height = int(width
                                         * (minsize['size'][1]/minsize['size'][0]))
                        else:
                            width = int(height
                                        * (minsize['size'][0]/minsize['size'][1]))
                elif size == 'min_width':
                    if self.panel._control_minsize:
        ...
        if self.icon_size:
            if self.icon_size[0] > 1:
                icon_width = int(self.icon_size[0])
                icon_height = int(self.icon_size[1])
                ...
                icon_width = int(width * self.icon_size[0])
                icon_height = int(height * self.icon_size[1])
                self.icon_size = None
        if (size in ('min', 'min_width') and
            self.panel._control_minsize and
            width <= self.panel._control_minsize['min'][0]):
    ...
    def set_control_image(self, control_image=None, data_folder=None,
        ...
        if control_image:
            ...
            if control_image[0] != 'none':
                data_folder, data_zip, color_key = self.panel._data_source(
                    data_folder, data_zip, color_key, file_obj)
                self.control_image['bg'] = self.panel._load_image(
                    control_image[0], path=data_folder,
                    zipobj=data_zip, fileobj=file_obj,
                    colorkey=color_key)
            else:
            ...
            if color_key:
                ...
                self.control_image['bg'].set_colorkey(color_key,
                                                      engine.RLEACCEL)
        else:
            try:
                self.control_image['bg'] = (
                    self.panel._image_default['control_image']['bg'].copy())
            except:
                if 'bg' in self.control_image:
                    del self.control_image['bg']
        if self.panel._initialized:
            ...
            self._define_buttons(self.control_type, self.size,
                                 self.color['normal'], self.color['fill'],
                                 initialize=False)
    ...
    def set_list(self, control_list, icon_list=None,
                 size=None, icon_size=None, case=False,
                 data_folder=None, data_zip=None, file_obj=None,
                 color_key=None, surface=None, append=False,
                 index=None, keep_link=True, keep_tip=True):
        """Set control listing of a control."""
        if not control_list:
            return
        if not size:
            size = self.size
        if control_list[0] not in ('__numeric_i',
                                   '__numeric_f',
                                   '__alpha',
                                   '__alphanumeric',
                                   '__filelist'):
            if append:
                ...
                if icon_list or surface:      #icon in listing
                    ...
                    if listing_icon:
                        control_icon = self._set_listing_icon(
                            listing_icon, icon_list, data_folder,
                            data_zip, file_obj, color_key, surface)
                        control_icon = self._set_icon_size(
                            control_icon, self.icon_size)
                        for icon in control_icon:
                ...
                size_old = self.size
                pos_old = (self.position[0] + self.size[0]//2,
                ...
                if icon_list or surface:
                    if not icon_size:
                        ...
                        self.icon_size = icon_size
                setting = self._set_listing(
                        control_list, icon_list, size, case, data_folder,
                        data_zip, file_obj, color_key, surface)
                self.listing = setting[0]
                self.value = setting[1]
                self.control_icon = setting[2]
                ...
                if self.size != size_old:
                    self.position = (pos_old[0] - self.size[0]//2,
                                     pos_old[1] - self.size[1]//2)
                    if not self.control_image:
                        self.control_outline = True
                    else:
                        self.control_outline = self.outline
        ...
        if self.listing:
            if (len(self.listing) > self.hold_response_set or
                (self.listing[0][:-2] == '__numeric' and
                 (abs(self.numeric['h']-self.numeric['l']) >
                  self.hold_response_set))):
                self.hold_response = self.hold_response_set
            else:
                self.hold_response = 0
        self.panel._update_panel = True

    def set_list_icon(self, control_list, icon_list=None,
                      data_folder=None, data_zip=None, file_obj=None,
                      color_key=None, surface=None, icon_size=None):
        ...
        control_icon = self._set_listing_icon(
            control_list, icon_list, data_folder,
            data_zip, file_obj, color_key, surface)
        if icon_size:
            if icon_size[0] > 1:
                icon_width = int(icon_size[0])
                icon_height = int(icon_size[1])
            else:
                icon_width = int(self.size[0] * icon_size[0])
                icon_height = int(self.size[0] * icon_size[1])
                if icon_width % 2:
                    icon_width += 1
                if icon_height % 2:
                    icon_height += 1
    ...
    def set_text_margin(self, margin):
        ...
        if isinstance(margin, int):
            self.text_margin['t'] = margin
            self.text_margin['r'] = margin
            self.text_margin['b'] = margin
            self.text_margin['l'] = margin
        else:
            self.text_margin['t'] = margin[0]
            self.text_margin['r'] = margin[1]
            self.text_margin['b'] = margin[2]
            self.text_margin['l'] = margin[3]
    ...
    def resize_control(self, size):
        ...
        pos_old = (self.position[0] + self.size[0]//2,
                   self.position[1] + self.size[1]//2)
        self.size, self.icon_size = self._set_control_size(
            self.listing, self.control_icon, size, self.font_size)
        if self.control_icon:
            self.control_icon = self._set_icon_size(
                self.control_icon, self.icon_size)
        self.position = (pos_old[0] - self.size[0]//2,
                         pos_old[1] - self.size[1]//2)
        self.button, self.rects = self._define_buttons(
            self.control_type, self.size,
        ...
        label_position = (self.position[0] + (self.size[0]//2),
                          self.position[1] - (label_size+3))
        self.label.set_position(label_position, center=True)
        self.panel._update_panel = True

    def set_display_info(self, size, font_color, font_type, font_size,
                         position=None, text=None, center=True):
        ...
        if not position:
            if center:
                ...
                position = (self.position[0] + (size[0]//2),
                            self.position[1] + (size[1]//2)-(height//2))
            else:
                position = self.position
        display.set_position(position)
        return display

    def set_display_text(self, font_color=None, font_bgcolor=None,
                         font_type=None, font_size=None, split_text=None,
                         size=None, info=False):
        ...
        if update_control:
            ...
            position = ( self.position[0] + (self.size[0]//2),
                         self.position[1] + (self.size[1]//2)-(height//2) )
    ...
    def set_label_info(self, size, font_color, font_type, font_size,
        ...
        if self.control_type == 'label':
            label.set_position((self.position[0],
                                self.position[1]-(font_size+3)),
                               center=True)
            return label
        if not position:
            position = (self.position[0] + (size[0]//2),
    ...
    def set_label_text(self, font_color=None, font_bgcolor=None,
                       font_type=None, font_size=None, split_text=None):
        ...
        if update_position:
            if self.label.split_text and self.label_text.count(' '):
                ...
                self.label.y = (self.position[1] -
                                (((self.label.font_size-2)*num_lines)+2))
            else:
                self.label.y = (self.position[1] -
                                (self.label.font_size+2))
        self.panel._update_panel = True
    ...
    def check_link(self, ctrl, activate):
        ...
        if control.link:
            if not activate:
                ...
                if ((control.control_type == 'function_select' and
                     control.link_activated) or
                    control.control_type == 'function_toggle'):
                    for ctr in control.link[control.value]:
    ...
    def _action_numeric_i(self, button):
        ...
        if button == self.id:
            ...
            if (greater(self.value, self.numeric['h'], self.numeric['step']) and
            ...
            self.activated = False
            self.value -= self.numeric['step']
            if (not greater(self.value, self.numeric['l'], self.numeric['step']) and
                self.value != self.numeric['l']):
                if self.loop:
                    self.value = self.numeric['h']
                else:
    ...
    def _action(self, button):
        ...
        if not self._functionObj:
            ...
            engine.util.call(self._functionObj, self._function,
                             (button,self.value))
...
class Label(Control):
    ...
    **Label Object**
    ...
    def _define_buttons(self, control_type, size, color, fill,
                        initialize=True):
        """Define control layout."""
        button = {}
        rects = {}
    ...
    def set_line_max(self, line=None):
        ...
        if line is None:
            box_size = (self.size[1]
                        - (self.text_margin['t']+self.text_margin['b']) + 2)
        ...
        if self.change:
            ...
            if not self.control_image:
                ...
                if not hasattr(self.image, 'clear'):
                    self.image = self.display.render(
                        engine.Surface(self.size, engine.SRCALPHA))
                else:
                    self.image.clear()
                    self.image = self.display.render(self.image)
            self.change = False
